...
*   **Conflict Resolution:** *(🚧 WIP: Needs detailed conflict resolution logic for edge cases where multiple offline updates occur)*.
...
*   **Voice-to-Text Input:** Integrate Speech-to-Text for foreman notes.
    *   *Implementation:* Web Speech API for the MVP (free/native), with an upgrade path to OpenAI Whisper API or Deepgram (Nova-3) for handling heavy accents and background construction noise.
