...
import { createProfile, getCurrentUser } from "../../src/services/apiUsers";
...
describe("getCurrentUser", () => {
  ...
  it("should GET /api/users/me with the access token as a Bearer header and return the data", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
        success: true,
        data: { id: "user-1", companyId: "company-1", role: "foreman", tier: "premium" },
      }),
    });
    vi.stubGlobal("fetch", fetchMock);

    await expect(getCurrentUser("token-123")).resolves.toEqual({
      id: "user-1",
      companyId: "company-1",
      role: "foreman",
      tier: "premium",
  ...
  it("should reject with the backend error message on an error response", async () => {
    ...
    await expect(getCurrentUser("token-123")).rejects.toThrow(
